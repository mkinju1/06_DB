...
/*1*/FROM EMPLOYEE
/*2*/GROUP BY DEPT_CODE; -- 같은 그룹끼리 묶음
-- D9, D6, D5, D9, NULL, D1, D2
...
SELECT DEPT_CODE, COUNT(*)   -- 4.조회 결과에 포함된 컬럼 지정
FROM EMPLOYEE		             -- 1.어떤 테이블에서
WHERE BONUS IS NOT NULL			 -- 2.어떤 행만 조회할지 지정
GROUP BY DEPT_CODE					 -- 3.WHERE 결과에서 그룹 그분
ORDER BY DEPT_CODE ASC;			 -- 5.조회 결과 정력
 ...
 * WHERE 와 HAVING 의 차이
 * 
 * WHER : FORM 절에서 지정된 테이블 전체 행 중
 * 				조건에 맞는 행만 골라내는 구문
 * 
 *  HAVING : GROUP BY를 통해 만들어진 그릅 중
 * 					 조건에 맞는 그룹만 골라내는 구문
 * */
...
ORDER BY DEPT_CODE ASC;
--> EMPLOYEE 테이블에서 DEPT_CODE 기준으로 만들어진 그룹 붕
